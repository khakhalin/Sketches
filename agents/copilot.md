...
4. Type a good first prompt. Here's an imaginary attempt of a "one-shot" prompt:

> We need to add a new transformer to the `UsefulTransformers` class. The context for this change is that we need to support a special case in which the field `global_time` is not available. We need to make sure the transformer can handle this case, while satisfying API calls from `Separator` classes. Please read files #file:summary.md for general info, then check out the code in files #file:code1.py, #file:code2.py and #file:code3.py. Dig into the raw code of ParentPackage to check how the base class for `pp.transformers.transformer` works. Finally, read the test #file:test_code1.py as we'll need to make sure it is still running. The original DoD for this change is copied below. Once you have read everything, come up with 2-3 options for how to design it, and output them in chat; I will instruct you on what design to implement. If you are missing any information, please ask. _(followed by an empty line, a sequence of --- to mark a start of a quotation, and a simple Ctrl-V or DoD from Jira, or some dump of design notes that you may have)_

As you can see, a good prompt is rather complex. It contains:

1. The motivation for the task (now that agents have access to the documentation, it may help them to frame the task correctly)
...
3. An explicit list of all files that _have_ to be read. You cannot include the whole repo in this list, but try to identify the key texts. If the files are currently open in the editor, you don't even have to type `file:` - you just add a hashtag and start typing the name of the file, then pick it from the list. Referencing the file by name without creating an active hashtag-driven link also works sometimes, but there's less guarantee.
4. Name relevant classes and methods literally as they are called in the code - that way the agent can grep them (search for them within the repo) and get extra context if necessary.
5. If you want the agent to dive into FF Core or PyrogAI code, mention it explicitly: as this code lives in different repos, it is less discoverable. It helps if you have fresh versions of these repos cloned on your machine, although with some effort agent can access the installed raw code as well.
...
7. Making the agent ask for help (for missing information) is harder than asking it to verify its plans with you; the example given above ("If you are missing any info...") usually fails. If you are positive that it _needs_ to get extra info from you to proceed, word it differently; write: "Before implementing anything, think of 3-9 critical pieces of information that you may be missing, and ask me about them in the chat. I will either answer you, or we will have a short back-and-forth before coming to the same page".
8. After the information-dense main prompt, you may try to add other "additional info" that may or may not be useful. But try not to distract the agent too much; even a slight mistake or obsolete info in this extra text may technically bias it towards a suboptimal implementation.
